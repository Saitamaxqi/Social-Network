...
func GetFollowStatuses(userID int) (map[int]*FollowStatus, error) {
	rows, err := DB.Query(`
	...
	followStatuses := make(map[int]*FollowStatus)
	for rows.Next() {
		var followID int
		var followingID int
		var status string
		err = rows.Scan(&followID, &followingID, &status)
		if err != nil {
			return nil, err
		}
		followStatuses[followingID] = &FollowStatus{ID: followID, Status: status}
	}
